...
start:
    ...
    ; Initialize File system from HDD image
    call initialize_file_system ; 
...
initialize_file_system:
    ; BPB (BIOS Parameter Block) for FAT32
    OEM_identifier      db 'MSWIN4.1'  ; OEM identifier
    Bytes_per_sector    dw 512        ; Bytes per sector
    Sectors_per_cluster db 8       ; Sectors per cluster
    Reserved_sectors    dw 1          ; Reserved sectors (including boot record)
    FAT_count           db 2                 ; Number of FATs
    Root_directory_entries dw 512  ; Number of root directory entries
    Total_sectors       dw 0             ; Total sectors in the logical volume (0 for now)
    Media_descriptor    db 0xF8       ; Media descriptor type
    Sectors_per_FAT     dw 32          ; Number of sectors per FAT (FAT12/FAT16 only)
    Sectors_per_track   dw 63        ; Number of sectors per track
    Heads_or_sides      dw 255          ; Number of heads or sides
    Hidden_sectors      dd 0            ; Number of hidden sectors (LBA of the beginning of the partition)
    Large_sector_count  dd 0        ; Large sector count (0 for now)
